...
# ---------------------------------------------------------------------------
# Full-scale evaluation from a single campaign result (with inline `drift`),
# including a HELD-OUT split so the threshold is not tuned on the test data.
# This directly answers the "no held-out set" limitation of the mini paper.
# ---------------------------------------------------------------------------
...
def _auroc(tags, d3, drift):
    bad = [drift[t] for t in tags if d3[t] > 0.0]
    good = [drift[t] for t in tags if d3[t] <= 0.0]
    if not bad or not good:
        return None
    pairs = [(b, g) for b in bad for g in good]
    return sum((b > g) + 0.5 * (b == g) for b, g in pairs) / len(pairs)


def evaluate_cara_holdout(campaign_result: dict, seed: int = 0) -> dict:
    """Evaluate CARA's drift front-end on a campaign result that carries an
    inline ``drift`` per row, with a tune/test split.

    Reports (1) the whole-set correlation and AUROC, and (2) a HELD-OUT
    protocol: choose the admission threshold on a tune half, then measure it on
    the untouched test half. This removes the "threshold picked on the same
    points it is judged on" weakness of the mini evaluation.
    """
    rows = [r for r in campaign_result["rows"] if r.get("drift") is not None]
    tags = [r["tag"] for r in rows]
    d3 = {r["tag"]: r["Delta3"] for r in rows}
    drift = {r["tag"]: r["drift"] for r in rows}
    coll = {r["tag"]: r.get("c3_pipe_collision_rate", 0.0) for r in rows}

    n = len(tags)
    n_bad = sum(1 for t in tags if d3[t] > 0.0)

    # ---- whole-set descriptive stats ----
    shift = {r["tag"]: (r.get("plan_shift3") or 0.0) for r in rows}
    spear = _spearman([drift[t] for t in tags], [shift[t] for t in tags])
    auc_all = _auroc(tags, d3, drift)
    sweep = sweep_tau(tags, d3, drift)
    bal_all = balanced_operating_point(sweep)
    cm_bal_all = _metrics(confusion_at_tau(tags, d3, drift, bal_all["tau"])) if bal_all else None

    # ---- held-out protocol: split tags into tune / test (deterministic) ----
    order = sorted(tags)                      # deterministic base order
    rng = random.Random(seed)
    rng.shuffle(order)
    half = len(order) // 2
    tune, test = order[:half], order[half:]

    # choose threshold on the tune half (balanced operating point)
    sweep_tune = sweep_tau(tune, d3, drift)
    bal_tune = balanced_operating_point(sweep_tune)
    tau_star = bal_tune["tau"] if bal_tune else 0.25

    # apply that fixed threshold to the untouched test half
    cm_test = _metrics(confusion_at_tau(test, d3, drift, tau_star))
    auc_test = _auroc(test, d3, drift)

    return {
        "n_updates": n, "n_bad": n_bad, "n_good": n - n_bad,
        "any_collision": any(coll[t] > 0.0 for t in tags),
        "whole_set": {
            "spearman_drift_vs_planshift": spear,
            "auroc_drift_vs_bad": auc_all,
            "balanced_operating_point": cm_bal_all,
        },
        "held_out": {
            "seed": seed,
            "tune_tags": tune, "test_tags": test,
            "tau_chosen_on_tune": tau_star,
            "test_confusion": cm_test,
            "test_auroc": auc_test,
        },
        "per_update": [
            {"tag": t, "drift": drift[t], "Delta3": d3[t],
             "plan_shift3": shift[t], "bad": d3[t] > 0.0}
            for t in tags
        ],
    }
