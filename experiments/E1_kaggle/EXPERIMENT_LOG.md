...
| 16 | Auto-running on Kaggle (user changed accelerator to T4 x2 via web UI; `labsd-src` v10 + saved kernel re-fired automatically) | First run with T4 (sm_75 — supported by torch 2.10's sm_70+ minimum). Same code path as v14/v15: real YOLOv11 fine-tune on Boston, baseline measurements, real fine-tune on Singapore, after-retrain measurements, Table I. CPU fallback in `c1_yolo` will kick in only if T4 still isn't allocated for some reason. |
