...
| 17 | Pushed (auto-running) | Same code as v16 + the `evaluate_c2` dispatch fix. Should complete the full pipeline: Boston YOLO fine-tune → baseline → Singapore YOLO fine-tune → after-retrain → Table I. **First run that should produce real cascade numbers.** |
